document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/students.csv')
        ...
        .then(text => {
            text.split(/\r?\n/).slice(1).forEach(row => {
                ...
                if (cols[0]) VALID_STUDENTS.add(cols[0].trim());
            });
            studentsLoaded = true;
        })
        .catch(() => alert('Student database failed to load'));
    ...
    submitBtn.onclick = () => {
        ...
        if (!capturedBlob) {
            alert('REQUIRED: Please capture a photo of the item before submitting.');
            return;
        }

        if (!studentNumber.value.trim() || !password.value || !itemName.value.trim() || !category.value || !location.value) {
            alert('Please complete all fields');
            return;
        }

        if (!VALID_STUDENTS.has(studentNumber.value.trim())) {
            alert('Invalid student number. Not found in records.');
            return;
        }
    ...
    confirmSubmitBtn.onclick = async () => {
        ...
        receiptBox.innerHTML = `
            <h3 style="text-align:center; letter-spacing:3px;">SUBMISSION RECEIPT</h3>
            ...
            <button class="submit-btn" id="goHomeBtn" style="margin-top:18px;">RETURN TO HOME</button>
        `;
